...
export function MetricItem({ title, count, Icon, onClick, hasPointer = false, isCurrency = false, isDoubleTitle = false, valueClassName = "", secondTitle = 0, FirstIcon, SecondIcon }: { 
    hasPointer?: boolean; 
    title: string; 
    count: number | string; 
    ...
    return <div onClick={onClick} className={`${hasPointer ? "cursor-pointer" : "cursor-auto"} rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] md:p-6`}>
        ...
        <div className="flex items-end justify-between mt-5">
            <div>
                ...
                {isDoubleTitle && <h3 className={`mt-2 font-bold text-title-sm ${valueClassName ? valueClassName : "text-gray-800 dark:text-white/90"}`}>
                    {isCurrency ? formatCurrency(Number(secondTitle)) : secondTitle} {SecondIcon && <SecondIcon className={`size-4 inline-block ml-1 ${valueClassName ? valueClassName : "text-gray-800 dark:text-white/90"}`} />}
                </h3>}
            </div>
        </div>
    </div>;
